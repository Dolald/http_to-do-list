...
func stringPointe(s string) *string {
	return &s
}
...
func TestTodoListPostgres_GetAll(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := r.GetAllLists(tt.input.userId)
			if tt.wantErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
...
func TestTodoListPostgres_Delete(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := r.DeleteList(tt.input.userId, tt.input.listId)
			if tt.wantErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
	...
	tests := []struct {
	...
	}{
		{
			...
			input: args{
				...
				userId: 1,
				input: todo.UpdateListInput{
					Title:       stringPointe("new title"),
					Description: stringPointe("new description"),
				},
			},
		},
		{
